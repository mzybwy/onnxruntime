...
namespace onnxruntime {
  ...
  static Node* PlaceNode(Graph& graph, std::unique_ptr<IndexedSubGraph> capability,
	...
	node->SetExecutionProviderType(provider_type);
      ...
      std::cout << "BEGIN CHECKING SUBGRAPH\n";
      for (auto node_index : capability->nodes) {
	auto node = graph.GetNode(node_index);
	
	std::cout << "Checking node: " << node->OpType() << "\n";
	...
	if (nullptr == node || !node->GetExecutionProviderType().empty()) {
	  ...
	  std::cout << " - sub-graph cannot be fused!\n";
	  break;
	}
      }
	...
	std::cout << " - fused node created: " << node_name << "\n";
  ...
  Status GraphPartitioner::Partition(Graph& graph, bool export_dll, FuncManager& func_mgr) const {
    ...
    for (auto& node : graph.Nodes()) {
      std::cout << x << " - GOT NODE: " << node.OpType() << " (" << node.Name() << ")\n";
      for (auto& entry : node.GetAttributeNameToMutableSubgraphMap()) {
	Graph* subgraph = entry.second;
	std::cout << x << " - GOT ENTRY: " << entry.first <<"\n";
