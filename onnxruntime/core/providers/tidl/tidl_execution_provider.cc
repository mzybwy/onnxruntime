...
namespace onnxruntime {
  ...
  TIDLExecutionProvider::TIDLExecutionProvider(const TIDLExecutionProviderInfo& info)
    ...
    auto default_allocator_factory = [](int) {
				       auto memory_info = onnxruntime::make_unique<OrtMemoryInfo>(TIDL, OrtAllocatorType::OrtDeviceAllocator);
    ...
    auto cpu_allocator_factory = [](int) {
				   auto memory_info = onnxruntime::make_unique<OrtMemoryInfo>(
											      TIDL_CPU, OrtAllocatorType::OrtDeviceAllocator, OrtDevice(), 0, OrtMemTypeCPUOutput);
				   return onnxruntime::make_unique<CPUAllocator>(std::move(memory_info));
				 };
  ...
  std::shared_ptr<KernelRegistry> TIDLExecutionProvider::GetKernelRegistry() const {
    static std::shared_ptr<KernelRegistry> kernel_registry = onnxruntime::tidl::GetTidlKernelRegistry();
    return kernel_registry;
  }
