...
#include "core/providers/tidl/subgraph/subgraph.h"
...
class TIDLExecutionProvider : public IExecutionProvider {
 public:
  ...
  std::shared_ptr<tidl::memory> GetWeightsMemoryBuffer(const std::string& weight_key) {
  ...
  void SetWeightsMemoryBuffer(const std::string& weight_key,
                              const std::shared_ptr<tidl::memory>& filter_dst_mem) {
  ...
  std::shared_ptr<tidl::memory> GetBiasMemoryBuffer(const std::string& key) {
  ...
  void SetBiasMemoryBuffer(const std::string& key,
                           const std::shared_ptr<tidl::memory>& bias_mem) {
 ...
 private:
  // tidl weights(filer data) memory blocks from first iteration
  // saved by weights name
  std::unordered_map<std::string, std::shared_ptr<tidl::memory>> weights_mem_map_;
  ...
  std::unordered_map<std::string, std::shared_ptr<tidl::memory>> bias_mem_map_;
 ...
 private:
  ...
  // Some dimensions are not supported by TIDL
  ...
  void CreateOrUpdateTidlNode(const Node* node,
  ...
  // Create Tidl node, update inputs, outputs and parent nodes
  // collect attribtes
 ...
 public:
  const std::shared_ptr<ort_tidl::Subgraph> GetTidlSubgraph(const std::string& subgraph_id) {
 ...
 private:
  ...
  // supported Tidl Operators
  std::set<std::string> tidl_ops_ = {
      // "Conv",
      // "BatchNormalization",
      // "Relu",
      // "Sum",
  ...
  mutable std::unordered_map<std::string, std::shared_ptr<ort_tidl::Subgraph>> mkl_subgraphs_;
};
